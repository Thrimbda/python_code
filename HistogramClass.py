...
def Histogram(data, yLabel, xLabel, ticks, title, plotNum=1):
    ...
    for i in np.arange(plotNum):
        rects.append(axs[i].bar(ticks, data[i], facecolor=(np.random.random(), np.random.random(), np.random.random()), edgecolor='white'))
...
if __name__ == '__main__':
    fig = Histogram(data, 'time(ms)', 'magnitude(10^x)', ticks, 'Select num index from disordered array')
    plt.show()
