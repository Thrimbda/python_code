...
class graph():
    def __init__(self, xmin, xmax, ymin, ymax):
        ...
        # 打开文件
        self.std_fobj = open(
            os.path.split(os.path.realpath(__file__))[0] + '/Fmt_PointRoute.txt', 'r')
        ...
        while not self.string:
            self.string = self.fobj.readline()
        self.type, self.state = tuple(eval(self.string))
        ...
        # self.database = self.fobj.readlines()[:50]
    ...
    # expand ax_x when t is larger than xlim
        ...
        try:
            self.type, self.info = tuple(eval(self.fobj.readline()))
        except Exception:
            pass
        if self.type == 'postrure':
        ...
        if self.t >= self.max:
            self.ax2.set_xlim(self.min, self.max + 100)
...
class GUIsetting(graph):  # 建立GUI设置类（以网络适配器配置类为基类）
    ...
    def make_widgets(self):  # 配置函数
        ...
        tk.Button(self.top, text='save PointRoute', command=self.clear).pack(
            side=tk.BOTTOM)  # 设置查看当前ip按钮并定位

    def clear(self):
        shutil.copyfile(os.path.split(os.path.realpath(__file__))[
                        0] + '/PointRoute.txt', os.path.split(os.path.realpath(__file__))[0] + '/2.1_route9.txt')
