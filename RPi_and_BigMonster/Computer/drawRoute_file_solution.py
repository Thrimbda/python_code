...
class graph():
    ...
    # def generator(self):		# 数据迭代器
        # for frames in self.database:
            # self.type, self.info = tuple(eval(frames))
            ...
            # self.t += 1
            # self.Angle_display.set_text('Angle = %.2f' %  (self.A*180/np.pi))
    ...
    def func(self, frames):		#绘图函数
        ...
        self.min, self.max = self.speed.get_xlim()
        if self.t >= self.max:
            self.angle.set_ylim(self.min, 2*self.max)
            self.speed_x.set_ylim(self.min, 2*self.max)
            self.speed_y.set_ylim(self.min, 2*self.max)
            self.speed.set_ylim(self.min, 2*self.max)
            self.angle.figure.canvas.draw()
            self.speed_x.figure.canvas.draw()
            self.speed_y.figure.canvas.draw()
            self.speed.figure.canvas.draw()
    ...
    def drawAni(self):
        self.draw = animation.FuncAnimation(self.fig, self.func, init_func = self.init, blit = True, interval = 1000)
        plt.show()
...
if __name__ == '__main__':
    fig1 = graph(-14000, 0, 0,14000, '192.168.10.90', 5005, 3)
    try:
        fig1.drawAni()
    finally:
        fig1.fobj.close()